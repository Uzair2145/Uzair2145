## Hi there! I'm Uzair

I am an international Business Finance and Economics undergraduate at the University of Manchester (2025-2029), aspiring to build a career as a strategic investor in asset management. My background combines rigorous quantitative problem solving backed by a perfect score of 100 in CBSE Mathematics - with hands-on audit experience and technical data analytics frameworks.

---

### 📊 Technical and Analytical Skills:
Financial Skills: Financial Reporting, Financial Decision Making, and Introductory Management Accounting.
Data and Coding: Python(Pandas, NumPy), MySQL, Advanced Excel (PivotTables, VLOOKUPs), Power BI ( Interactive Dashboards), and MS Office.
AI & Certifications: Google AI Essentials, Microsoft & LinkedIn Career Essentials in Generative AI, Edx Economic Development coursework.


### 💼 Professional & Practical Experience:
Audit Intern | Shah & Alshamali Associates (July 2026 - August 2026): Audited 400+ financial transactions (payroll, VAT, ADCB bank reconciliations) ensuring strict data integrity, and reconciled discrepancies within Zoho databases to enhance financial reporting accuracy.
Data Analytics Training | Time Training Centre (July 2025- Sept 2025): Applied advanced tools to clean, manipulate, and model large datasets for quantitative evaluations.


### ☘️  Leadership and Community Engagement:
Social Media Manager | University of Manchester Indian Dance Society: Managing publicity, creating digital assets (trailers/posters), and collaborating on large-scale university events.
Teaching & Outreach Volunteer | AngelXpress Foundation & WeBiz: Tutored students of grade 6-7 in mathematics over a 41-hour commitment and hosted workshops, sessions, and courses engaging over 100 students to expand commerce education.  
Community Volunteer | Parivarthan Foundation and Kitab Organisation (November 2025): Supported Community paper drive initiatives by collection and routing 1600kg+ of recyclable waste to processing facilities to promote environement sustainability.


---


### 📧 Let's Connect
LinkedIn: https://www.linkedin.com/in/mohammed-uzair-didgur-1815512b8/
Location: Manchester, UK | Abu Dhabi, UAE

 



